#!/usr/bin/env bash
# SLURM job: run ESC-50 official inference on a CPU node against a running server URL.
#
# NOTE: This is an exception workflow. The default project policy is to run inference
# locally (hybrid pattern). Use this only when explicitly requested.
#
# Submit:
#   sbatch examples/slurm/run_esc50_official_cpu_inference_naturelm_v1_0.sh
#
...
#
# Defaults assume serve job 56627 wrote:
#   $HOME/beans-next-launchers/56627.url

#SBATCH --partition=cpu
#SBATCH --cpus-per-task=4
#SBATCH --time=2:00:00
#SBATCH --export=ALL
#SBATCH --output="/home/%u/logs/%A_infer_esc50_v10_cpu.log"
#SBATCH --job-name="infer-esc50-v10-cpu"
...
URL_FILE_DEFAULT="$HOME/beans-next-launchers/56627.url"
...
OUT="/home/$USER/beans-next-results/raw/esc50_official_v10_full_cpu_${SLURM_JOB_ID}_esp_data"
mkdir -p "$OUT"
...
uv run beans-next run \
  ...
  --limit 400 \
  --run-id "esc50_official_v10_full_cpu_${SLURM_JOB_ID}_${BEANS_NEXT_DATA_SOURCE:-esp_data}" \
  -o "$OUT"
