#!/usr/bin/env bash
# Submit BEANS-Next Tier-3 sampling run (20 examples/task) for NatureLM-audio v1.1.
#
# Usage (from repo root):
#   bash examples/slurm/submit_beans_next_t3_naturelm_v1_1.sh
#
# Uses GCS checkpoint: gs://foundation-models/naturelm-audio-1.1/base_model/1290000
...
# Standard v1.1 checkpoint (non-multiaudio).
...
echo "Submitting serving job (NatureLM v1.1 ckpt=1290000, h100-80)..."
SERVE_JOB=$(
  HF_TOKEN="${HF_TOKEN:-}" \
  HUGGINGFACE_HUB_TOKEN="${HUGGINGFACE_HUB_TOKEN:-}" \
  NATURELM_GCS_CHECKPOINT_URI="$NATURELM_GCS_CHECKPOINT_URI" \
  BEANS_NEXT_DEBUG=1 \
  sbatch --parsable --partition=h100-80 examples/slurm/serve_naturelm_v1_1.sh
)
