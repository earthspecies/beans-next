#!/usr/bin/env bash
# SLURM job: start OpenAI-compatible proxy (Gemini upstream) on a CPU node.
#
...
#   sbatch examples/slurm/serve_openai_proxy_gemini.sh
#
...
#
# Notes:
# - Secrets: store keys in `~/.config/gemini/cfg` (preferred) or `~/.config/openai/cfg`.
# - Network: requires egress from the compute node to `generativelanguage.googleapis.com`.
#
#SBATCH --partition=cpu
#SBATCH --cpus-per-task=2
#SBATCH --export=ALL
#SBATCH --output="/home/%u/logs/%A.log"
#SBATCH --job-name="beans-next serve openai-proxy gemini"
...
PORT="${BEANS_PRO_PORT:-19086}"

# Job-scoped uv environment on node-local scratch.
export UV_PROJECT_ENVIRONMENT="${BEANS_PRO_UV_PROJECT_ENVIRONMENT:-/scratch/$USER/venvs/beans-next-gemini-proxy-${SLURM_JOB_ID}}"
...
# Gemini proxy settings (recommended conservative caps for stability/cost).
...
export OPENAI_MODEL="${OPENAI_MODEL:-gemini-2.5-pro}"
export OPENAI_PROXY_CANONICALIZE_WAV="${OPENAI_PROXY_CANONICALIZE_WAV:-1}"
export OPENAI_PROXY_MAX_AUDIO_SECONDS="${OPENAI_PROXY_MAX_AUDIO_SECONDS:-30}"
...
export GEMINI_MIN_MAX_TOKENS="${GEMINI_MIN_MAX_TOKENS:-1024}"
...
def main() -> int:
    ...
    port = int(os.environ.get("PORT", "19086"))
    ...
    # Real-mode prediction smoke-check (Gemini can be slow; use a longer timeout).
    import base64
    import io
    import wave
    ...
    try:
        ...
        print(
            f"Launcher smoke-check failed: /predict request error: {exc!r}",
            file=sys.stderr,
        )
        return 1
    if status != 200:
    ...
    if not isinstance(preds, list) or not preds or not all(isinstance(x, str) for x in preds):
        print(
            f"Launcher smoke-check failed: empty/invalid predictions: {item!r}",
            file=sys.stderr,
        )
        return 1
