...
URL_FILE="${BEANS_NEXT_URL_FILE:-}"
if [[ -z "$URL_FILE" ]]; then
  echo "ERROR: BEANS_NEXT_URL_FILE must be set to the Qwen launcher .url file." >&2
  exit 2
fi
CONFIG_PATH="${BEANS_NEXT_CONFIG:-configs/benchmarks/birdset_core_qwen3_omni_esp_data.yaml}"
INC="${BEANS_NEXT_INCREMENT:-i30}"
RUN_ID="${BEANS_NEXT_RUN_ID:-full_qwen3_omni_instruct_2_h100_birdset_core_r1}"
OUT_DIR="${BEANS_NEXT_OUT_DIR:-/scratch/${USER}/.cache/beans-next-results/${INC}/qwen3_omni_instruct_2/birdset_core/${RUN_ID}}"

SERVE_JOB="${BEANS_NEXT_SERVE_JOB_ID:-}"
if [[ -z "$SERVE_JOB" ]]; then
