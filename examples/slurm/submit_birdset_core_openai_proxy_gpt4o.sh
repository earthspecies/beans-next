#!/usr/bin/env bash
# Submit a full BirdSet core evaluation run via the OpenAI-compatible proxy (GPT-4o audio).
#
# Usage (from repo root on Slurm login node):
#   bash examples/slurm/submit_birdset_core_openai_proxy_gpt4o.sh
#
# Optional overrides:
#   BEANS_NEXT_OUT_DIR=/my/path bash examples/slurm/submit_birdset_core_openai_proxy_gpt4o.sh
#
# Notes:
# - Requires network egress from the proxy node to api.openai.com.
# - Secrets are loaded by the serve script from ~/.config/openai/cfg (do not echo keys).
# - BirdSet uses esp_data: this script sets BEANS_NEXT_DATA_SOURCE=esp_data.

set -euo pipefail

RUN_TAG="birdset_core_gpt4o_proxy_$(date +%Y%m%d_%H%M%S)"
OUT_DIR="${BEANS_NEXT_OUT_DIR:-${BEANS_PRO_OUT_DIR:-${SCRATCH:-$HOME}/beans-next-results/$RUN_TAG}}"

echo "Submitting proxy serving job..."
SERVE_JOB="$(sbatch --parsable examples/slurm/serve_openai_proxy_gpt4o.sh)"
